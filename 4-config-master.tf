resource "null_resource" "control-plane-config" {
  ...
  provisioner "file" {
    ...
    connection {
      type        = "ssh"
      user        = var.user
      host        = module.master_domain.address[count.index].ip
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo sh -c  \"echo '${module.master_domain.address[count.index].ip} ${module.master_domain[count.index].name}' > /etc/hosts\""
    ]
    connection {
      type        = "ssh"
      user        = var.user
